defmodule GH.GraphQL do
  ...
  connection into a lazy `Stream`, mirroring `GH.REST.stream/3` so both transports
  paginate through one mental model.

  ## Result shape

  `query/3` returns `{:ok, data, meta}` on success, where `data` is the contents of
  the GraphQL `data` field and `meta` is a `GH.GraphQL.Meta`. GraphQL answers with
  HTTP 200 even on failure, so a response carrying a non-empty `errors` array
  becomes `{:error, %GH.Error{}}`; any partial `data` is preserved on the error's
  ...
  serialize to the matching `$name` in the query.
      ...
      GH.GraphQL.query(client, ~s|query($login: String!) {
  ...
  nodes. Tell `stream/4` where the connection lives with `:path`:
      ...
      client
      |> GH.GraphQL.stream(query, [org: "joshrotenberg"], path: ["organization", "projectsV2"])
      |> Stream.map(& &1["title"])
      |> Enum.to_list()
